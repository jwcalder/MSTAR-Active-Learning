...
UNCERTAINTY_METHODS = ["norm", "entropy", "least_confidence", "smallest_margin", "largest_margin", "random"]
...
if __name__ == "__main__":
    ...
    for unc_method in UNCERTAINTY_METHODS:
        print(f"Uncertainty Method = {unc_method.upper()}")
        ...
        train_ind, accuracy = active_learning_loop(W, evals, evecs, train_ind, labels, args.iters, "uncertainty", uncertainty_method = unc_method,  all_train_idx=train_idx_all, test_mask=test_mask, gamma=args.gamma, algorithm=args.algorithm)

        results_df[unc_method+"_choices"] = np.concatenate(([-1], train_ind[-args.iters:]))
        results_df[unc_method+"_acc"] = accuracy
